...
test('test color mix', () => {
  expect(fancyColor('#fad').mix('#ddd', 40).toHexString()).toBe('#f1bedd');
  expect(fancyColor('#fad').shade(40).toHexString()).toBe('#996684');
  expect(fancyColor('#fad').tint(40).toHexString()).toBe('#ffddf1');
...
test('test input color error', () => {
  expect(() => { fancyColor('#gggggg'); }).toThrow('unsupported color type: #gggggg');
  expect(() => { fancyColor('rgb(322, 1, 22)'); }).toThrow('rgb() color input error: rgb(322, 1, 22), please check the input');
  expect(() => { fancyColor('rgba(109, 1, 22, 20)'); }).toThrow('rgba() color input error: rgba(109, 1, 22, 20), please check the input');
  expect(() => { fancyColor('hsl(109, 1, 232)'); }).toThrow('hsl() color input error: hsl(109, 1, 232), please check the input');
  expect(() => { fancyColor('hsla(109, 1, 22, 20)'); }).toThrow('hsla() color input error: hsla(109, 1, 22, 20), please check the input');
  expect(() => { fancyColor({ r:255, g: 1270, b:221 }); }).toThrow('rgb color input error: {\"r\":255,\"g\":1270,\"b\":221}, please check the input');
  expect(() => { fancyColor({ h: 324, s:100, l:123.3 }); }).toThrow('hsl color input error: {\"h\":324,\"s\":100,\"l\":123.3}, please check the input');
});
